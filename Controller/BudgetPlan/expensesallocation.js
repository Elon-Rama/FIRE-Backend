...
// controllers/ExpensesAllocation.js
const ExpensesAllocation = require('../../Model/ExpensesAllocation');
...
exports.upsert= async (req, res) => {
  try {
    ...
    const totalExpenses = Titles.reduce((sum, title) => sum + (title.amount || 0), 0);

    const updatedAllocation = await ExpensesAllocation.findOneAndUpdate(
      { userId, month: currentMonth, year: currentYear }, // Filter by userId, current month, and year
      {
        userId,
        month: currentMonth,
        year: currentYear,
      ...
      { upsert: true, new: true, setDefaultsOnInsert: true } // upsert creates a new document if none matches
    );

    res.status(200).json({
      statusCode: "0",
      message: "Expenses Allocation created/updated Successfully",
      allocation_id: updatedAllocation._id,
      userId: updatedAllocation.userId,
      Expenses: {
        month: updatedAllocation.month,
        year: updatedAllocation.year
      },
      Titles: updatedAllocation.Titles,
      totalExpenses: updatedAllocation.totalExpenses
    });
  } catch (error) {
    console.error("Error creating/updating Expenses Allocation:", error);
