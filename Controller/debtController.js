...
//     // Monthly interest calculation based on outstanding balance
//     const monthlyInterestRate = loan.interest / 100 / 12;
...
exports.getAllDebts = async (req, res) => {
  ...
  try {
    ...
    debtClearance.source.forEach((loan) => {
      ...
      totalPaid += loan.currentPaid;
      totalOwed += loan.totalPayment - loan.currentPaid;
      loan.outstandingBalance = Math.round(loan.outstandingBalance); // Ensure rounding here
    });
...
exports.payEMI = async (req, res) => {
  try {
    ...
    loan.outstandingBalance = Math.round(loan.outstandingBalance); // Round after update
